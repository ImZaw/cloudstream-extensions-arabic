...
class Movizland : MainAPI() {
    ...
    private fun String.cleanTitle(): String {
        val regex = """/انمي|مترجمة|مسلسل|فيلم|مشاهدة|وتحميل|مترجم|عرض|اون لاين|إنمي/g""".toRegex()
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        ...
        val title = select("div.BlockTitle").text().cleanTitle().replace("$year","")
    ...
    private fun Element.toEpisode(): Episode {
        ...
        if(EpiSt){
            ...
            val title = select("h2 > span > a").text().cleanTitle()
        ...
        return newEpisode(url) {
            ...
            val url = a.select("a")?.attr("href")
            val title = a.select("div.BlockTitle").text().cleanTitle()
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val movtitle = doc.select("h2.postTitle").text()
        val titleEl =  doc.select("ul.ButtonsFilter.WidthAuto li.active").isNotEmpty()
        val titleSt = if(titleEl) true else false
        val sertitle = if(titleSt){doc.select("ul.ButtonsFilter.WidthAuto li.active").text().cleanTitle()}else{doc.select("div.BottomBarRecent > div > h2 > span > a").text().cleanTitle()}
        val isMovie = if(movtitle.contains("عرض|فيلم".toRegex())) true else false
        val title = if(isMovie){movtitle}else{sertitle}
        .cleanTitle().replace("$year","")
        val posterUrl = sdetails.select("img")?.attr("data-src")
        val presynp = movtitle.toRegex()
        val synop = doc.select("section.story").text().replace(presynp,"")
        val synopsis = if(synop.startsWith(" : ")){synop.replace(" : ".toRegex(),"")}else{synop}
        ...
        val recmovies = doc.select("div.RecentItems .BlockItem").map { element ->
            MovieSearchResponse(
                ...
                name = element.select(".BlockTitle").text().cleanTitle(),
